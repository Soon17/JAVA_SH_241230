# 카테고리 상의를 클릭했을 때 조회하는 쿼리(키를 알 때, "상의"를 알 때, "TOP"을 알 때)
SELECT *
FROM PRODUCT
WHERE PR_CA_NUM = 1;
...
SELECT PRODUCT.*, IFNULL(SUM(BL_AMOUNT), 0) AS 판매수
FROM PRODUCT
...
SELECT *
FROM (
	SELECT DENSE_RANK() OVER(ORDER BY 판매수 DESC) AS RNK, BUY_PRODUCT.*
    FROM (
		SELECT PRODUCT.*, IFNULL(SUM(BL_AMOUNT), 0) AS 판매수
        FROM PRODUCT
		LEFT JOIN BUY_LIST ON PR_CODE = BL_PR_CODE 
		GROUP BY PR_CODE) AS BUY_PRODUCT
        ) AS T
WHERE RNK = 1;

# 카테고리별 최고 금액을 조회하는 쿼리 
SELECT PR_CA_NUM AS 카테고리, MAX(PR_PRICE) 최고금액
FROM PRODUCT
GROUP BY PR_CA_NUM;
...
FROM PRODUCT
JOIN (
		SELECT PR_CA_NUM AS 카테고리, MAX(PR_PRICE) AS 최고금액
		FROM PRODUCT
        GROUP BY PR_CA_NUM) AS T
ON PR_CA_NUM = 카테고리
WHERE PR_PRICE = 최고금액
ORDER BY 카테고리;

# 사용자별 누적 구매액을 조회하는 쿼리
# - OUTER JOIN : 구매내역이 없어도 출력 
# - GROUP BY, SUM() : 사용자별 구매액 계산 
SELECT ME_ID, IFNULL(SUM(BU_TOTAL_PRICE), 0) AS 누적구매액
FROM BUY

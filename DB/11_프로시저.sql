...
1. 변수선언 : 변수는 프로시저 시작위치에 모아둠. 중간 선언 불가.
...
6. 반복문 - REPEAT
	...
	UNTIL 조건식
	END REPEAT;
7. 반복문 - CURSOR : 검색 결과를 반복문으로 활용할 때
	DECLARE 변수명A BOOLEAN DEFAULT FALSE;
	DECLARE 커서명 CURSOR FOR SELECT 속성들 FROM 테이블명;
...
# 등록된 학생만큼 AVERAGE 테이블에 1학년 1학기 ~ 3학년 2학기 평균 초기 데이터를 추가하는 프로시저
...
# AVERAGE 테이블에 SUM과 COUNT를 모두 0으로 초기화하는 프로시저
...
BEGIN
    ...
    CURSOR_LOOP : LOOP
		...
		SET
			...
			AV_COUNT = AV_COUNT + 1
		WHERE
			AV_GRADE = _SC_GRADE AND
            AV_SEMESTER = _SC_SEMESTER AND
            AV_ST_KEY = _SC_ST_KEY;
	END LOOP;
    CLOSE _CURSOR;
END ?!
DELIMITER ;

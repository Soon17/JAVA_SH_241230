...
public class StudentManager {
	...
	public boolean updateStudent(Student selStd, Student newStd) {
		...
		if(selStd.equals(newStd)) return studentDao.updateStudent(newStd, newStd);
		
		//학년, 반, 번호가 다른 경우
		//새 학생 정보가 중복된 경우
		if(contains(newStd)) return false;
	...
	public int getScoreNum(Student std, SubjectScore subjectScore) {
		...
		if(std == null || subjectScore == null || subjectScore.getSubject() == null) return -1;
		
		//등록된 학생인지 확인
		Student dbStd = studentDao.selectStudent(std);
		if(dbStd == null) return -1;
		
		//등록된 과목인지 확인
		Subject dbSubject = subjectDao.selectSubject(subjectScore.getSubject());
		if(dbSubject == null) return -1;
	...
	public boolean insertScore(Student std, SubjectScore subjectScore) {
		if(std == null || subjectScore == null) return false;
		
		if(getScoreNum(std, subjectScore) != -1) return false;
	...
	public boolean deleteScore(Student std, Subject subject) {
		if (std == null || subject == null) return false;
		Student dbStd = studentDao.selectStudent(std);
		if(dbStd == null) return false;

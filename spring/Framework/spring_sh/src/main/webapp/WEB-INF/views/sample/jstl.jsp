...
<body>
	...
	<c:if test="${age >= 20 }">
		<h3>(c:if) 20이상이므로 성인입니다.</h3>
	</c:if>
	
	<!-- {}안에서 공백이 몇개 들어가도 상관없지만, "와 $사이에 공백이 들어가면 정상적으로 동작하지 않음 -->
	<c:if test="${age <    20 }">
		<h3>(c:if) 20미만이므로 성인이 아닙니다.</h3>
	</c:if>
	
	<c:choose>
		<c:when test="${age >= 20 }">
			<h3>(c:when) 20이상이므로 성인입니다.</h3>
		</c:when>
		<c:otherwise>
			<h3>(c:when-otherwise)그렇지 않으므로 성인이 아닙니다.</h3>
		</c:otherwise>
	</c:choose>
	...
	<ul>
		<c:forEach items="${list}" var="fruit" varStatus="vs">
			<li>
				...
				<c:if test="${vs.first}">FIRST</c:if>
				<c:if test="${vs.last}">LAST</c:if>
			</li>
		</c:forEach>
	</ul>
